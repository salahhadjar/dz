...
function askForRDV(timestamp, skey){
	...
	//ajaxCall("ajax_form_status", ajaxCall("ajax_form_status", "https://pt.tlscontact.com/dz/ALG/action.php?process=multiconfirm&what=book_appointment&fg_id=5627222&result="+event.target.innerText+"&issuer_view=dzALG2pt");
...
function displayConfirmModal( date, timestamp, skey ){
	...
	if( !ModalExist ){
		...
		var f = document.createElement('form');
			...
			f.setAttribute( "action", "https://pt.tlscontact.com/dz/ALG/action.php");
...
function ajaxPost( date, timestamp, skey ){
	...
	// set target url 
	var targetUrl = "https://"+window.location.hostname+"/dz/ALG/action.php";
